...
public class Room
{
    ...
    public void setExit(String d, Room r) {
        Door door = new Door(this, r, false, true);
        exits.put(d,door);
    }
    ...
    public Room getExit(String d) throws Exception {
        Door requestedDoor = exits.get(d);
        if (requestedDoor.isLocked()) {
            throw new Exception("The door is locked!");
        }
        return requestedDoor.nextRoom;
    }

    public Door getDoor(String d) {
        return exits.get(d);
    }
    ...
    public Map<String, Room> getExits() {
        Map<String, Room> convertedMap = new HashMap<>();
        for (String key : exits.keySet()) {
            convertedMap.put(key, exits.get(key).nextRoom);
        }
        return convertedMap;
    }
}
